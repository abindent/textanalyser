            ...
            <div className="max-w-3xl mx-auto relative z-10">
                ...
                <Card className="overflow-hidden flex items-center ">
                    ...
                    {blog.poster_img_url && (
                        <div className="relative w-full h-96">
                            <Image
                                ...
                                priority
                                sizes="(max-width: 568px) 80vw, 700px"
                            />
                        </div>
                    )}
                    ...
                    <div className="p-6 md:p-10">
                        ...
                        {/* Markdown Content - Enhanced Renderer */}
                        <div className="prose prose-sky dark:prose-invert max-w-none prose-headings:font-bold prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-a:text-sky-600 dark:prose-a:text-sky-400 prose-code:bg-slate-100 dark:prose-code:bg-slate-800 prose-pre:bg-slate-900 prose-img:rounded-lg prose-img:shadow-lg">
                            <ReactMarkdown
                                ...
                                components={{
                                    h1: ({ node, ...props }) => (
                                        <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl mb-4 mt-6" {...props} />
                                    ),
                                    h2: ({ node, ...props }) => (
                                        <h2 className="scroll-m-20 border-b text-3xl font-semibold tracking-tight transition-colors mb-3 mt-5 pb-2" {...props} />
                                    ),
                                    h3: ({ node, ...props }) => (
                                        <h3 className="scroll-m-20 text-2xl font-semibold tracking-tight mb-2 mt-4" {...props} />
                                    ),
                                    h4: ({ node, ...props }) => (
                                        <h4 className="scroll-m-20 text-xl font-semibold tracking-tight mb-2 mt-3" {...props} />
                                    ),
                                    p: ({ node, ...props }) => (
                                    ...
                                    blockquote: ({ node, ...props }) => (
                                        <blockquote className="mt-6 border-l-2 border-sky-500 pl-6 italic text-slate-600 dark:text-slate-400" {...props} />
                                    ),
                                    code: ({ node, inline, ...props }: any) =>
                                        inline ? (
                                            <code className="relative rounded bg-slate-100 dark:bg-slate-800 px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold" {...props} />
                                        ) : (
                                            <code {...props} />
                                        ),
                                    pre: ({ node, ...props }) => (
                                    ...
                                    img: ({ node, ...props }) => (
                                        <img className="rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg" {...props} />
                                    ),
                                    table: ({ node, ...props }) => (
                                    ...
                                    tr: ({ node, ...props }) => (
                                        <tr className="border border-slate-300 dark:border-slate-600 even:bg-slate-50 dark:even:bg-slate-800" {...props} />
                                    ),
                                    td: ({ node, ...props }) => (
                                        <td className="border border-slate-300 dark:border-slate-600 px-4 py-2" {...props} />
                                    ),
                                    th: ({ node, ...props }) => (
                                        <th className="border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 px-4 py-2 font-bold" {...props} />
                                    ),
                                    div: ({ node, ...props }) => {
                                        const id = (node as any)?.data?.hProperties?.id;
                                        return (
                                            <section id={id} className="my-8 p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700" {...props} />
                                        );
                                    },
                                    // Handle hr elements
                                    hr: ({ node, ...props }) => (
